...
class PythonCyc:
      ...
      def send_query(self, query):
           self.soc.sendall(query)

      def retrieve_results(self): 
           _data = self.soc.recv(9024)

           data =  self.parseLisp(_data)
           return data

      def getOrganismList(self):
         patterns = [ re.compile(r'^ECOBASES'),\
                    re.compile(r'^[#@]'), re.compile(r'BASE$')
                    ]
...
if __name__=="__main__":
    ...
    query= "(mapcar #'object-name (all-orgs :all))"
    pythonCyc = PythonCyc()
    pythonCyc.makeSocket()

    pythonCyc.getOrganismList()

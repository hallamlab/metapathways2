...
if (defined $HELP or !(defined $LIST or defined $OUTFILE)) {
    die "Usage: $0 [options] -o output.csv PGDB
Extracts information from a PGDB, producing a csv file describing
the base pathways of the database.

Note: Pathway Tools *must* be started with the -api argument:
    /path/to/pathway-tools -api
This creates a unix socket named /tmp/ptools-socket that this program uses
to run commands.  If it's not present, this program can't work.
...
my @subpathways = $cyc->get_slot_values("|Pathways|", 'Sub-Pathways');

print Dumper(@subpathways);


my @my_base_pathways = $cyc->call_func("Pathways :all T");
print Dumper(@my_base_pathways);
exit(0);

foreach my $pathway (@my_base_pathways) {
  my @mygenes = $cyc->genes_of_pathway($pathway,"T");
  print OUT join("\t",$pathway,cleanup($cyc->get_slot_value($pathway,"common-name")),scalar(@mygenes),(map {($cyc->get_slot_value($_, "common-name"))||"?"} @mygenes)),"\n";
}
close(OUT);

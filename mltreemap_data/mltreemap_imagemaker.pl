...
sub concatenate_files {
    ...
    open (IN, "$dirname/data/tree_data/available_dataset_colors.txt") or die "ERROR: data/tree_data/available_dataset_colors.txt does not exist!\n";
    <IN>;
    my $count = 0;
    while (<IN>) {
        chomp $_;
...
sub get_analysis_info {
    ...
    open (IN, "$dirname/data/tree_data/drawing_info.txt") or die "Error, can't open drawing_info.txt!\n";
    while (<IN>) {
        chomp $_;
	    #print "Drawing ".$_."\n";
        my ($dn, $tn) = split /\t/;
...
sub get_picture_dimensions {
    ...
    open (IN, "$dirname/data/tree_data/$tree_file") or die "Error, can't open $tree_file!\n";
    my $species_count = 0;
    while (<IN>) {
        chomp $_;
        while (/\d+:/g) {
...
sub get_support_data {
    ...
    open (IN, "$dirname/data/tree_data/$$mltreemap_results{tax_ids_file}") or die "cannot read 'data/tree_data/$$mltreemap_results{tax_ids_file}'!\n";
...
sub read_tree_topology {
    ...
    my $tree_file = "/data/tree_data/$$mltreemap_results{tree_file}";
